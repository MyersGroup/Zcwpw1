...
cons <- ggplot(jalview[human_aa], aes(1:sum(human_aa),scale(JSD))) + geom_col(width=1, fill="black") + theme_minimal() + xlab("Amino acid number") + ylab("Jensen-Shannon\n divergence (normalised)")

pdf("../results/Zcwpw1_domain_architechture_cons.pdf", width = 10, height = 8)
cowplot::plot_grid(p + theme(legend.position = "none"), cons, ncol=1, align = "v",axis = "lrtb", rel_heights = c(2,1), labels = "AUTO")
dev.off()
