...
ROC_PR <- plot_grid(pmain, get_legend(p1), ncol=1, rel_heights = c(1, 0.07))

#pdf(width=10, height = 5) #"../results/DMC1_ROC_PR.pdf", 
ROC_PR
#dev.off()
```
...
```{r DMC1_density, fig.width=10, fig.height = 5}
...
ggplot(HP9combo_peaks[cov_input>5 & FC_Zcw_CVC_cov_input>5], aes(FC_Zcw_CVC_enrichment+0.01, color=isDMC1)) +
  geom_density() +
  scale_x_log10(limits=c(0.01,10)) + ggtitle("Cterm")
...
p1 <- ggplot(HP9combo_peaks[cov_input>5 & FC_Zcw_CVC_cov_input>5], aes(FC_Zcw_CVC_enrichment, fill=isDMC1)) +
    ...
    scale_x_log10(limits=c(1e-2, 5)) +  theme_minimal() + theme(legend.position = "bottom") + guides(fill=guide_legend("Overlaps DMC1")) +
  scale_fill_brewer(palette = "Set1") + xlab("Enrichment of ZCWPW1 (with vs without human PRDM9)")

p2 <- ggplot(HP9combo_peaks[cov_input>5 & FC_Zcw_CVC_cov_input>5], aes(enrichment, fill=isDMC1)) +
    geom_density(alpha=0.5) + #, position="fill"
    scale_x_log10(limits=c(0.5, 50)) + theme_minimal() +
  scale_fill_brewer(palette = "Set1") + xlab("Enrichment of human PRDM9") 

density_comparison <- plot_grid(p1 + theme(legend.position = "none"), p2 + theme(legend.position = "none"), rel_widths = c(1, 1.25))
density_comparison
```

```{r Density_and_AUC, fig.width=10, fig.height = 10}
plot_grid(plot_grid(density_comparison, get_legend(p1), ncol=1, rel_heights = c(1, 0.07)),
          ROC_PR, nrow = 2, labels = NULL)
```
