...
```{r}
# Export beds
# Now done with snakemake pipeline
...
names(zcw_wP9_peaks_cin_QCM50) <- names(zcw_wP9_peaks_cin)[1:11]
zcw_wP9_peaks_cin_QCM50 <- prejoin(zcw_wP9_peaks_cin_QCM50)
...
set_matrix_zcw_peaks$Experiment <- "Zcw_Alone"

set_matrix_zcw_wP9_peaks_cin <- create_set_matrix(zcw_wP9_peaks_cin, k=100)
set_matrix_zcw_wP9_peaks_cin$Experiment <- "Zcw_P9_Cotransfection"

# set_matrix_zcw_wP9_peaks_cin_QCM50 <- create_set_matrix(zcw_wP9_peaks_cin_QCM50, k=100)
# set_matrix_zcw_wP9_peaks_cin_QCM50$Experiment <- "Zcw_P9_Cotransfection QCM50"

set_matrix_zcw_wP9_vs_chip_Z <- create_set_matrix(zcw_wP9_vs_chip_Z, k=100)
set_matrix_zcw_wP9_vs_chip_Z$Experiment <- "Zcw_P9_Cotransfection_VS_Zcw_alone"
...
proportions <- set_matrix_comb[,.("Not Zcw Alone" = sum(Zcw_Alone==0) / .N,
                             ...
                             "Zcw Alone, Not hP9, + Not ut_H3K4 + NotAlu + not NonAluRep" = sum(Zcw_Alone==1 & hP9_Cterm==0 & H3K4me3_UT==0 & Alu==0 & NonAlu==0) / .N,
                             "Zcw Alone, Not hP9, + Not ut_H3K4 + NotAlu + NonAluRep" = sum(Zcw_Alone==1 & hP9_Cterm==0 & H3K4me3_UT==0 & Alu==0 & NonAlu==1) / .N,
...
proportions_plot <- ggplot(tmp, aes(decile, value, fill=Subset, colour=Subset)) +
  ...
  ggtitle(paste0("For peaks with cov_in>5, overlap (CI+/-250) with other seperately called peaks (not force called)"))
...
which repeat most enriched at top

```{r}
tmp <- foverlaps(zcw_wP9_peaks_cin,repeats[!grep("Alu",Name)], mult="first")

tmp[ , decile := cut(i.enrichment,
                  breaks = quantile(i.enrichment, 
                                    probs = seq(0, 1, by = 1/k)),
                  labels = 1:k,
                  include.lowest=T)]

ggplot(merge(tmp[decile==1][,.N,by=Name], tmp[decile==100][,.N,by=Name], by="Name"), aes(N.x,N.y)) + geom_point() + scale_x_log10() + scale_y_log10() + geom_abline()
...
#wget https://www.encodeproject.org/files/ENCFF101AKQ/@@download/ENCFF101AKQ.bed.gz
#wget https://www.encodeproject.org/files/ENCFF690WNQ/@@download/ENCFF690WNQ.bed.gz

ctcf <- loadBed("~/Dropbox/Downloads/ENCODE_beds/ENCFF314ZAL.bed")
ctcf_2 <- loadBed("~/Dropbox/Downloads/ENCFF338RAX.bed") # bed narrowPeak	peaks and background as input for IDR	1, 2

# https://www.encodeproject.org/experiments/ENCSR000EUZ/ SE36nt HEK293T
Trim28 <- loadBed("~/Dropbox/Downloads/ENCODE_beds/ENCFF860DHS.bed") #, 29k peaks
Trim28_2 <- loadBed("~/Dropbox/Downloads/ENCFF959SPJ.bed") #300k
#bed narrowPeak	peaks and background as input for IDR	1, 2

#https://www.encodeproject.org/experiments/ENCSR474CVP/ PE100bp K562
Trim28_3 <- loadBed("~/Dropbox/Downloads/ENCFF131RPK.bed") # 300k

#https://www.encodeproject.org/experiments/ENCSR876GXA/ ZBTB33
zbtb33 <- loadBed("~/Dropbox/Downloads/ENCFF108BVL.bed")

#https://www.encodeproject.org/experiments/ENCSR142YYA/ EWSR1
ewsr1 <- loadBed("~/Dropbox/Downloads/ENCFF418KWK.bed")

h3K27ac <- loadBed("~/Dropbox/Downloads/ENCODE_beds/ENCFF451UZW.bed")
h3K27ac_2 <- loadBed("~/Dropbox/Downloads/ENCFF464QPC.bed")

# https://www.encodeproject.org/experiments/ENCSR000AKQ/, h3K27me3
h3K27me3 <- loadBed("~/Dropbox/Downloads/ENCFF700RBU.bed")

#https://www.encodeproject.org/experiments/ENCSR000FCG/
h3k4me1 <- loadBed("~/Dropbox/Downloads/ENCODE_beds/ENCFF483QXH.bed")

# https://www.encodeproject.org/experiments/ENCSR396QWK/ FLAG MBD
# HepG2 genetically modified using CRISPR, SE50nt
mbd1 <- loadBed("~/Dropbox/Downloads/ENCFF446OZF.bed")

# https://www.encodeproject.org/experiments/ENCSR221GAN/ K562 MBD2
mbd2 <- loadBed("~/Dropbox/Downloads/ENCFF786NME.bed")

# https://www.encodeproject.org/experiments/ENCSR000EVX/ ZNF274
znf274 <- loadBed("~/Dropbox/Downloads/ENCFF478NGK.bed")

#https://www.encodeproject.org/experiments/ENCSR987PBI/ DNMT1 K562
dnmt1 <- loadBed("~/Dropbox/Downloads/ENCFF129ADK.bed") # 300k peaks

# https://www.encodeproject.org/experiments/ENCSR156CWW/ dnmt3b in HepG2 FLAG tagged, SE52bp
dnmt3b <- loadBed("~/Dropbox/Downloads/ENCFF204MYY.bed") # 300k peaks

# https://www.encodeproject.org/experiments/ENCSR000BNK/
ctcfl <- loadBed("~/Dropbox/Downloads/ENCFF678TFE.bed") #CTCFL ChIP-seq protocol v041610.1 on human K562 
ctcfl_2 <- loadBed("~/Dropbox/Downloads/ENCFF081QMM.bed") #CTCFL bed narrowPeak	peaks and background as input for IDR	1, 2

# hg19
# system("cut -f 1-3 ~/Dropbox/Downloads/ENCODE_beds/ENCFF001VRG.bed > ~/Dropbox/Downloads/ENCODE_beds/ENCFF001VRG.bed3")
# setdb1 <- loadBed("~/Dropbox/Downloads/ENCODE_beds/ENCFF001VRG_hg38.bed3")

# hg38
# https://www.encodeproject.org/experiments/ENCSR000AUT/ SE36nt
setdb1_2 <- loadBed("~/Dropbox/Downloads/ENCFF101AKQ.bed") # ENCSR000AUT/ K562 300k peaks, peaks and background as input for IDR	1

# https://www.encodeproject.org/experiments/ENCSR000EWI/ SE32nt
setdb1_3 <- loadBed("~/Dropbox/Downloads/ENCFF611CFB.bed") # ENCSR000EWI/ K562 300k peaks, peaks and background as input for IDR	1, 2

h3k9 <- loadBed("~/Dropbox/Downloads/ENCODE_beds/ENCFF342JBJ.bed")
H3K9me3 <- loadBed("~/Dropbox/Downloads/ENCFF538EDC.bed") # K562, 92k peaks, bed narrowPeak	peaks	1
H3K9me3_2 <- loadBed("~/Dropbox/Downloads/ENCFF422AIH.bed") # HEK293, 54k peaks,  bed narrowPeak	peaks	2


setdb1_3$enrichment <- setdb1_3$V7


CGI <- fread("~/Dropbox/Downloads/cpgIslandExtUnmasked.txt") #CpG
names(CGI)[2:4] <- c("chr","CI_start","CI_stop")
CGI <- prejoin(CGI, ext = 250)

system("cut -f 2-4 cpgIslandExtUnmasked.txt > cpgIslandExtUnmasked.bed")
...
zcw_peaks_cin_m24 <- fread("~/Downloads/Zcwpw1_peak_cin_24bpmappable.bed")
...
IPSC_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF295WQL.bed")  #    Homo sapiens GM23338 originated from GM23248 https://www.encodeproject.org/experiments/ENCSR293NAJ/

H1C_H4K20 <- loadBed("~/Dropbox/Downloads/ENCODE_beds/ENCFF665UWA.bed") #     Homo sapiens H1 https://www.encodeproject.org/experiments/ENCSR728SZE/

H1C_H4K20_rep <- loadBed("~/Dropbox/Downloads/ENCFF439CWL.bed") #     Homo sapiens H1 https://www.encodeproject.org/experiments/ENCSR728SZE/

H9C_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF280SGN.bed") #    Homo sapiens hepatocyte originated from H9 https://www.encodeproject.org/experiments/ENCSR289TGF/
H9CSMO_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF653UGW.bed") #     Homo sapiens smooth muscle cell originated from H9 https://www.encodeproject.org/experiments/ENCSR189XIE/
H9CNP_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF435BYC.bed") # Some,   Homo sapiens neural progenitor cell originated from H9 https://www.encodeproject.org/experiments/ENCSR720ZAX/

IMR90_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF567BLE.bed") # up to 37, Homo sapiens IMR-90 https://www.encodeproject.org/experiments/ENCSR804SRO/

MCF7_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF151LTX.bed") #    Homo sapiens MCF-7, https://www.encodeproject.org/experiments/ENCSR639RHG/
HCT116_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF403LQH.bed") #     Homo sapiens HCT116 https://www.encodeproject.org/experiments/ENCSR474DOV/

KMS11_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF720TFF.bed")  #     Homo sapiens KMS-11 https://www.encodeproject.org/experiments/ENCSR790QWJ/

H1NC_H4K20 <- loadBed("~/Dropbox/Downloads/ENCFF108PRX.bed") #Homo sapiens neural cell originated from H1 https://www.encodeproject.org/experiments/ENCSR958ZMM/
...
peakAnno <- annotatePeak("Zcw_alone_unexplained.bed", tssRegion=c(-3000, 3000),
                         TxDb=txdb, annoDb="org.Hs.eg.db")

peakAnno2 <- annotatePeak("Zcw_alone_explained.bed", tssRegion=c(-3000, 3000),
                         TxDb=txdb, annoDb="org.Hs.eg.db")
...
```{r}
fraction_overlapN2(CP9combo_peaks[cov_input>=5], zcw_peaks, x1="cPrdm9 Cterm combined", y1="Zcw UT", shift = 1e9)
...
```{r}
fraction_overlapN2(HP9N_peaks[cov_input>=5], zcw_peaks, x1="hPrdm9 Nterm", y1="Zcw UT", shift = 1e9)
...
fraction_overlapN2(zcw_peaks[cov_input>=5], dmc1, x1="Zcw UT", y1="Dcm1", shift = 1e8, n=100)
fraction_overlapN2(zcw_peaks[cov_input>=5], dmc1, x1="Zcw UT", y1="Dcm1", shift = 1e8, n=100) + ylim(0,0.05)
...
fraction_overlapN2(zcw_wP9_peaks_cin_QCM50[cov_input>=5],  dmc1, x1="Zcw wP9 QCM50", y1="Dcm1", shift = 1e8, n=300)
```
...
```{r}
p1 <- fraction_overlapN2(zcw_wP9_peaks[cov_input>=5], h3k4me3_P9_peaks, x1="Zcw wP9", y1="h3k4me3_P9_peaks", shift=1e8, n=100) +
  ylab("Fraction of Zcwpw1 with Prdm9 peaks \n that overlap H3K4me3 with Prdm9 Peaks") +
  xlab("Mean Enrichment of Zcwpw1 cotrasnfected with Prdm9") +
  theme(legend.position = "bottom")

p2 <- fraction_overlapN2(zcw_wP9_peaks[cov_input>=5], h3k36me3_P9_peaks, x1="Zcw wP9", y1="h3k36me3_P9_peaks", shift=1e8, n=100) +
...
tmp2 + 
  ...
  ylab("Fraction of Zcwpw1 peaks \n that overlap Alu repeats")
  


tmp <- overlapGam(zcw_wP9_peaks[cov_input>=5], h3k36me3_P9_peaks, "a","b")


tmp$overlap <- !is.na(tmp$test_enrichment)

tmpgam <- gam(data=tmp, overlap ~ s(enrichment), family="quasibinomial")
tmpglm <- glm(data=tmp, overlap ~ enrichment, family="binomial")

n=500
x_new <- seq(0.01, 50, length.out = n)
y_pred <- data.table(enrichment = x_new,
                     data.frame(predict(tmpgam, data.frame(enrichment = x_new), se.fit=TRUE, type="response")))
tmpdt <- cbind(y_pred, Type="GAM")
setnames(tmpdt, c("mean_enrichment","Overlap","se","Type"))

y_pred2 <- data.table(enrichment = x_new,
                     data.frame(predict(tmpglm, data.frame(enrichment = x_new), se.fit=TRUE, type="response")))
tmpdt2 <- cbind(y_pred2, Type="GLM")
setnames(tmpdt2, c("mean_enrichment","Overlap","se","resid","Type"))


ggplot(y_pred, aes(enrichment, fit)) +
    geom_ribbon(data=cbind(y_pred, Type="GAM"), aes(ymin = fit - 2*se.fit, ymax = fit + 2*se.fit), fill = "grey70", colour="grey70") +
    geom_line() +
    theme_minimal()

tmp2 <- fraction_overlapN2(zcw_peaks_cin[cov_input>=5], repeats[Name!="Alu"][grep("Alu",Name)][CI_stop-CI_start>250 & CI_stop-CI_start<350], x1="Zcw UT", y1="Alu Repeats", shift=1e8) # yes up to 70%

tmp2 + 
  geom_ribbon(data=tmp, aes(ymin = Overlap - 2*se, ymax = Overlap + 2*se), fill = "grey70", colour="grey70", alpha=0.5) +
  scale_x_log10(limits=c(0.3,5))

p2 + 
  geom_ribbon(data=tmpdt, aes(ymin = Overlap - 2*se, ymax = Overlap + 2*se), fill = "grey70", colour="grey70", alpha=0.5) +
  geom_ribbon(data=tmpdt2, aes(ymin = Overlap - 2*se, ymax = Overlap + 2*se), fill = "grey70", colour="blue", alpha=0.5) +
  scale_x_log10(limits=c(0.5,15))
```
