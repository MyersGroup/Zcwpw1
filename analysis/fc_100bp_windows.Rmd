...
gzip -d cpgIslandExtUnmasked.txt.gz
cut -f 2-4 cpgIslandExtUnmasked.txt > cpgIslandExtUnmasked.bed

bedtools intersect -a forcepeaks/genome.windows.100wide.100slide.bed -b cpgIslandExtUnmasked.bed -f 0.1 -c > cpgIslandExtUnmasked_100bpwindows.bed

bedtools intersect -a forcepeaks/genome.windows.100wide.100slide.bed -b cpgIslandExtUnmasked.bed -f 1 -c > cpgIslandExtUnmasked_100bpwindows_f1.bed


##### DeNovo Motif

# count seqkit located motifs by 100bp
bedtools intersect -a forcepeaks/genome.windows.100wide.100slide.bed -b motif_locs_M7.bed -c > motif_locs_M7_100bpwindows.bed
bedtools intersect -a forcepeaks/genome.windows.100wide.100slide.bed -b motif_locs_CHB6.bed -c > motif_locs_CHB6_100bpwindows.bed

# count fimo located motifs by 100bp
awk '$5>13' fimo_M4_WG.tsv | cut -f 1-3 | tail -n +2 | 
bedtools intersect -a forcepeaks/genome.windows.100wide.100slide.bed -b stdin -c > fimo_M4_WG_100bpwindows.bed
...
wget ftp://ftp.ncbi.nlm.nih.gov/geo/samples/GSM1254nnn/GSM1254259/suppl/GSM1254259%5FHEK293%2DCT%2Ecout%2Etxt%2Egz

# only CpG meth
zcat GSM1254259_HEK293-CT.cout.txt.gz | awk ' $4 == "CG"' > GSM1254259_HEK293-CT.CpG.txt

# sum two strands for each CpG (so ignoring asymetric site)
awk -v OFS='\t' '
  NR%2 { split($0, a) ; next }
  {print a[1], a[2], $2, a[7]+$7, a[8]+$8, (a[6]+$6)/2}
' GSM1254259_HEK293-CT.CpG.txt > GSM1254259_HEK293-CT.CpG_Sum.txt

# remove high copynumber cpgs
awk '$6<1.5' GSM1254259_HEK293-CT.CpG_Sum.txt | cut -f 1-5 > GSM1254259_HEK293-CT.CpG_Sum_lowCN.txt

# hg19 to hg38
liftOver GSM1254259_HEK293-CT.CpG_Sum_lowCN.txt hg19ToHg38.over.chain.gz GSM1254259_HEK293-CT.CpG.hg38.bed GSM1254259_HEK293-CT.CpG.nonhg38.bed

# Careful, liftover creates overlaping (duplicate CpG locations)

# sort
sort -k1,1 -k2,2n GSM1254259_HEK293-CT.CpG.hg38.bed > GSM1254259_HEK293-CT.CpG.hg38.sort.bed
...
bedtools map -c 4,5 -o sum -null 0 -a cpgIslandExtUnmasked.bed -b GSM1254259_HEK293-CT.CpG.hg38.sort.bed > cpgIsland_Meth.bed

# sum for each 100bp
bedtools map -c 4,5 -o sum -null 0 -a forcepeaks/genome.windows.100wide.100slide.namesort.bed -b GSM1254259_HEK293-CT.CpG.hg38.sort.bed > cpg_Meth_100bpwindows.bed

awk -v OFS='\t' '{print $1,$2,$3,($4+1)/($4+$5+2)}' GSM1254259_HEK293-CT.CpG.hg38.sort.bed > GSM1254259_HEK293-CT.CpG.hg38.sort.mean.bed

# for IGV figure
awk '$1=="chr1" && $2>53800000 && $2<54000000' GSM1254259_HEK293-CT.CpG.hg38.sort.mean.bed > GSM1254259_HEK293-CT.CpG.hg38.sort.mean.chr1.bed

bedGraphToBigWig GSM1254259_HEK293-CT.CpG.hg38.sort.mean.chr1.bed ../single-cell/sequencing/metadata/hg38_sizes.chrom GSM1254259_HEK293-CT.CpG.hg38.sort.mean.chr1.bigWig
...
zcw_peak <- fread("peaks/SingleBasePeaks.WTCHG_538916_221156_vs_WTCHG_538916_217108_AND_WTCHG_538916_220144.p0.000001.sep250.ALL.100bpWindows.bed")
setnames(zcw_peak, c("chr","center_start","center_stop","ZcwPeak"))

# bedtools shuffle -chrom -seed 71346 -noOverlapping -i peaks/SingleBasePeaks.WTCHG_538916_221156_vs_WTCHG_538916_217108_AND_WTCHG_538916_220144.p0.000001.sep250.ALL.100bpWindows.bed -g hg38_sizes23.bed > peaks/Zcw_shuffle.bed
# ERROR ../single-cell/sequencing/metadata/hg38_sizes.chrom
...
zcw_peak_rand <- fread("peaks/Zcw_random_1bp.100bpWindows.bed")
...
```{r}
cpgImeth <- fread("cpgIsland_Meth.bed")
...
H3K4UT_CGI <- fread("peaks/ForceCalledPeaks_NA15-SRR5627150_AND_NA15-SRR5627151_vs_NA15-SRR5627142_AT_cpgIslandExtUnmasked.bed.bed")
Zcw_CGI <- fread("peaks/ForceCalledPeaks_WTCHG_538916_221156_vs_WTCHG_538916_217108_AND_WTCHG_538916_220144_AT_cpgIslandExtUnmasked.bed.bed")
...
ggplot(forcecalledk[zcw_cov_input>=5], aes(CpG, zcw_enrichment+0.01)) +
  geom_hex(aes(fill = stat(log(count+0.1))), bins=200) + theme(legend.position = "bottom") +
  scale_fill_viridis_c(direction = -1) + scale_y_log10()

ggplot(forcecalledk[meth+unmeth > 10 & zcw_cov_input>=5], aes(prop_meth, zcw_enrichment+0.01)) +
  geom_hex(aes(fill = stat(log(count+0.1))), bins=200) + theme(legend.position = "bottom") +
  scale_fill_viridis_c(direction = -1) + scale_y_log10()
...
# %, prior 0.5
# awk -v OFS='\t' '{print $1,$2,$3, $4+1 / ($4+$5+2);}' GSM1254259_HEK293-CT.CpG_Sum.txt > GSM1254259_HEK293-CT.CpG_percent.txt

# awk -v OFS='\t' '$1 == "chr1" && $2>22000000 && $2<22600000 {print $1,$2,$2+1,$6;}' GSM1254259_HEK293-CT.CpG.txt > CpGMeth_CN_sample.bed
# bedGraphToBigWig hglft_genome_56cb7_9415b0.bed ../single-cell/sequencing/metadata/hg38_sizes.chrom CpGMeth_CN_sample.bigWig
# 
# awk -v OFS='\t' '$1 == "chr1" && $2>22000000 && $2<22600000 {print $1,$2,$2+1,$7 / ($7+$8+1);}' GSM1254259_HEK293-CT.CpG.txt > CpGMeth_sample.bed
# # liftover
# awk -v OFS='\t' '{print $1, $2, $3, 1-$4}' hglft_genome_aa0c_8e5a20.bed > hglft_genome_aa0c_8e5a20_UnMeth.bed
# bedGraphToBigWig hglft_genome_aa0c_8e5a20_UnMeth.bed ../single-cell/sequencing/metadata/hg38_sizes.chrom CpGUnMeth_sample.bigWig
# bedGraphToBigWig hglft_genome_aa0c_8e5a20.bed ../single-cell/sequencing/metadata/hg38_sizes.chrom CpGMeth_sample.bigWig
...
M7 <- fread("motif_locs_M7_100bpwindows.bed")
...
h3k36me3_UT_100bp <- fread("peaks/ForceCalledPeaks_NA15-SRR5627148_vs_NA15-SRR5627142_AT_genome.windows.100wide.100slide.bed.bed")
...
hP9GFP <- fread("peaks/ForceCalledPeaks_NA15-SRR5627138_AND_NA15-SRR5627139_vs_NA15-SRR5627140_AT_genome.windows.100wide.100slide.bed.bed")
...
forcecalled$CpG_Island_any <- forcecalled$CpG_Island

CpG_Island_100bp <- fread("cpgIslandExtUnmasked_100bpwindows.bed")
setnames(CpG_Island_100bp, c("chr","center_start","center_stop","CpG_Island"))
forcecalled <- merge(forcecalled, CpG_Island_100bp)

CpG_Island_100bp <- fread("cpgIslandExtUnmasked_100bpwindows_f1.bed")
...
cpg_Meth_100bp <- fread("cpg_Meth_100bpwindows.bed")
...
fimo <- fread("fimo_M4_WG_100bpwindows.bed")
setnames(fimo, c("chr","center_start","center_stop","M4"))
setkey(fimo, chr,center_start,center_stop)
forcecalledM4 <- merge(fimo, forcecalled)

quantile(forcecalledM4$M4, c(0.25,0.5,0.75,0.85,0.95,0.98,0.99,0.999,0.9999))

max(forcecalledM4$M4)

forcecalledM4[M4==117]

seqkit <- fread("motif_locs_CHB6_100bpwindows.bed")
setnames(seqkit, c("chr","center_start","center_stop","M4"))
setkey(seqkit, chr,center_start,center_stop)
forcecalledCHG <- merge(seqkit, forcecalled)
...
forcecalledk[, methStatus := "Unk"]
forcecalledk[prop_meth>0.75, methStatus := "Meth"]
forcecalledk[prop_meth<0.25, methStatus := "UnMeth"]

pPeak_CpG <- rbind(
forcecalledk[CpG_Island>0,.(PeakProb=mean(ZcwPeak),.N, Type="Observed"), by=c("CpG","methStatus")][order(CpG)],
forcecalledk[CpG_Island>0,.(PeakProb=mean(ZcwPeakRand>0),.N, Type="Random"), by=c("CpG","methStatus")][order(CpG)])
...
saveRDS(p,"NormalPeaks300bp.rds")
...
zcw_lm7 <- lm(zcw_enrichment ~ h3k4me3_UT_enrichment + h3k36me3_UT_enrichment + CpG + CpG_Island + prop_meth, forcecalledk[zcw_cov_input>5])

zcw_lm8 <- lm(zcw_enrichment ~ h3k4me3_UT_enrichment + h3k36me3_UT_enrichment + CpG * prop_meth  + M7 + CpG_Island, forcecalledk[zcw_cov_input>5])

zcw_lm9 <- lm(zcw_enrichment ~ h3k4me3_UT_enrichment * h3k36me3_UT_enrichment + CpG * prop_meth  + M7 + CpG_Island, forcecalledk[zcw_cov_input>5])

zcw_lm10 <- lm(zcw_enrichment ~ h3k4me3_UT_enrichment * h3k36me3_UT_enrichment , forcecalledk[zcw_cov_input>5])


zcw_lm11 <- lm(zcw_enrichment ~ h3k4me3_UT_enrichment + h3k36me3_UT_enrichment + CpG + sqrt(M4), forcecalledM4[zcw_cov_input>5])
```
...
ggsave("CpG_Proportions_windows.pdf", width = 7, height = 5)
...
regions <- fread("forcepeaks/qPCRregions_18-09-19.bed")
...
```{r}
forcecalledk$enrichment <- forcecalledk$prop_meth

tmp <- rbind(
  fraction_overlapN(forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][CpG_Island==0],
...
forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][CpG_Island==0][prop_meth<0.1][order(-zcw_enrichment)]
...
forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][, .(mean(CpG), quantile(CpG, 0.25), quantile(CpG, 0.75), quantile(CpG, 0.95)), by=CpG_Island]

forcecalledk[zcw_cov_input>5][CpG==0][meth>0]

 

tmp <- rbind(
  fraction_overlapN(forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][CpG_Island==0][CpG==1],
                    "prop_meth","zcw_enrichment", n=25, returndf = T, filter="CpG==1", pseudoenrich = 0.01),
  fraction_overlapN(forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][CpG_Island==0][CpG==2],
                    "prop_meth","zcw_enrichment", n=25, returndf = T, filter="CpG==2", pseudoenrich = 0.01),
  fraction_overlapN(forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][CpG_Island==0][CpG==4],
                    "prop_meth","zcw_enrichment", n=25, returndf = T, filter="CpG==4", pseudoenrich = 0.01),
  fraction_overlapN(forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][CpG_Island==0][CpG==7],
                    "prop_meth","zcw_enrichment", n=25, returndf = T, filter="CpG==7", pseudoenrich = 0.01),
  fraction_overlapN(forcecalledk[zcw_cov_input>5 & (meth + unmeth>10)][CpG_Island==1][CpG==7],
...
dmc1 <- fread("Pratto_human_DSB_Aintersect_autosomal_hg38.bed")
...
mappability <- fread("mappability/hg38.75mer.Excludable.t0.5.chr1.merge500.bed")
