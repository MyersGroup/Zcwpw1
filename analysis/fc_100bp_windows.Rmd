...
CpG_100bp <- fread("grep -P 'chr1\\t|chr\\t' CpG_hg38_100bpwindows.bed")
...
# log scale
ggplot(forcecalled[zcw_cov_input>5 & hP9combo_cov_input>5], (aes(log(zcw_enrichment+0.1), log(zcw_hP9_enrichment+0.1), z=hP9Combo.bound))) +
...
forcecalled[cP9combo_pvalue<1e-4, cP9Combo.bound := TRUE]

ggplot(forcecalled[zcw_cov_input>5 & cP9combo_cov_input>5], (aes(zcw_enrichment, zcw_cP9_enrichment, z=cP9Combo.bound))) +
  stat_summary_hex(bins=200, alpha=0.5, fun= function(x) as.character(mean(x))) + # na.rm=T
  #scale_fill_brewer(palette="Set1") +
  scale_fill_manual(labels = c("p = 1", "p < 10^(-4)"), values = c("#E41A1C", "#377EB8")) +
  labs(fill="Evidence of cPrdm9cterm binding") +
  theme(legend.position = "bottom") +
  ylim(0,8)
...
forcecalled[hP9combo_cov_input>5 & hP9combo_pvalue<1e-7, UTP9 := "1"] #& h3k4me3_UT_pvalue==1
forcecalled[hP9combo_cov_input>5 & hP9combo_pvalue<1e-7 & h3k4me3_UT_pvalue<1e-5, UTP9 := "2"]
#forcecalled[hP9combo_pvalue>0.1 & h3k4me3_UT_pvalue<1e-5, UTP9 := "3"]
...
commongrobs <- list(
  stat_summary_hex(bins=150, alpha=0.5, fun= function(x) if(length(x)>=1){as.character(mean(x))}else{NA}),
  theme(legend.position = "bottom"),
  scale_fill_manual(labels = c("No hP9 (p=1)", "hP9 (p<1e-7) & No UT_H3K4 (p>1e-5)","hP9 (p<1e-7) & UT_H3K4 (p<1e-5)"), values = RColorBrewer::brewer.pal(3, "Set1")),
  xlim(NA,4),
  ylim(NA,2.5)
)
...
pmain <- plot_grid(p1 + commongrobs + theme(legend.position = "none"),
          p3 + commongrobs + theme(legend.position = "none"),
          p4 + commongrobs + theme(legend.position = "none"),
          p2 + commongrobs + theme(legend.position = "none"))
...
fraction_overlapN <- function(dt,x,y, n=25, filters=c("All"), returndf=F){
  ...
  return(
    ggplot(tmp, aes(mean_enrichment, Mean, colour=Subset, group=Subset)) +
      #geom_point(size=2) +
...
tmp <- rbind(
  fraction_overlapN(forcecalled[h3k4me3_hP9_cov_input>5 & zcw_cov_input>5][UTP9==1], 
                    "h3k4me3_hP9_enrichment","zcw_hP9_enrichment", n=25, returndf = T, filter="UTP9==1"),
  fraction_overlapN(forcecalled[h3k4me3_hP9_cov_input>5 & zcw_cov_input>5][UTP9==2], 
...
ggplot(tmp, aes(mean_enrichment, Mean, colour=Subset)) +
  geom_point() +
  scale_x_log10() +
  xlab("Mean h3k4me3_hP9_enrichment") +
  ylab("Mean zcw_hP9_enrichment") +
  geom_pointrange(aes(ymax = uq, ymin = lq)) +
  geom_errorbar(aes(ymax = Mean + 2*sem, ymin = Mean - 2*sem), width = 0.05) +
  scale_colour_manual(labels = c("hP9 (p<1e-7) & No UT_H3K4 (p>1e-5)",
                               "hP9 (p<1e-7) & UT_H3K4 (p<1e-5)"),
