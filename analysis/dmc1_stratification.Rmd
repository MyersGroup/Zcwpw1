---
title: "DMC1 Profiles, stratified by heat"
output: html_notebook
---


```{r}
B6 <- fread("dmc1/B6_composite_filtered.bed")
names(B6) <- names(fread("dmc1/B6_composite.txt", nrows = 1))

bwm <- as.matrix(fread("dmc1/ZCWPW1_HOM_260619_5prime_atB6.bwm"))
bwm_wt <- as.matrix(fread("dmc1/B6_Sample1_Brick2012_5prime_atB6.bwm"))

bwm3p <- as.matrix(fread("dmc1/ZCWPW1_HOM_260619_3prime_atB6.bwm"))
bwm_wt3p <- as.matrix(fread("dmc1/B6_Sample1_Brick2012_3prime_atB6.bwm"))
...
spo11 <- as.matrix(fread("dmc1/B6_Spo11_atB6.bwm"))
...
plot_profile <- function(a,b,c=NULL,d=NULL,e=NULL,f=NULL, left=650, right=1150, ylabel=NULL, name=NULL, ggplot=F){
  ...
  p <- ggplot(tmp, aes(pos, y, colour=Y, group=interaction(Genotype,Y), linetype=Genotype)) +
    ...
    xlab("Position relative to motif (kb)") +
    ylab(paste("Relative DMC1 signal")) +
  ...
  plot(pos, a, type="l",ylim=c(0,max(c(a,b,c,d))), xlim=c(pos[left],pos[right]), xlab="Position relative to motif (kb)", ylab=ylabel)
...
saveRDS(p2, "DMC_stratified.rds")

plot_profile(bwm[conds[,1],], bwm[conds[,2],], bwm[conds[,3],], left=650, right=1150, ggplot=TRUE)
pdf("H3K4me3_gg.pdf")
p2
dev.off()
