...
zcw_ST_unexplained_M3 <- findamotif(head(zcw_ST_unexplained_sm,1000), len=7, nits=200, seed = 42)
...
plot_moitf <- function(motif, name){
  ...
  print(ggplot(melt(data.table(motif$alphas, seq=1:200), id="seq"), aes(seq, value, colour=variable)) + geom_line())
...
fraction_overlapN2(zcw_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="Zcw UT", y1="M7_locs", shift=1e8)
fraction_overlapN2(HP9combo_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="HP9combo", y1="M7_locs", shift=1e8)
fraction_overlapN2(HP9N_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="HP9N", y1="M7_locs", shift=1e8)
fraction_overlapN2(CP9combo_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="CP9combo", y1="M7_locs", shift=1e8)



tmp <- zcw_peaks_cin[cov_input>5][chr!="chrX"][order(-enrichment)][set_matrix_zcw_peaks3[,which(Zcw_Alone==T & hP9_Cterm==F & H3K4me3_UT==F & Alu==F)]][chr=="chr1"]
fraction_overlapN2(tmp, M4_locs, x1="Unexplained Zcw UT", y1="M7_locs", shift=1e8)
```
...
fraction_overlapN2(zcw_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="Zcw UT", y1="CNN7_locs", shift=1e8)
fraction_overlapN2(HP9combo_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="HP9combo", y1="CNN7_locs", shift=1e8)
fraction_overlapN2(HP9N_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="HP9N", y1="CNN7_locs", shift=1e8)
fraction_overlapN2(CP9combo_peaks[cov_input>=5 & chr=="chr1"], M4_locs, x1="CP9combo", y1="CNN7_locs", shift=1e8)



tmp <- zcw_peaks_cin[cov_input>5][chr!="chrX"][order(-enrichment)][set_matrix_zcw_peaks3[,which(Zcw_Alone==T & hP9_Cterm==F & H3K4me3_UT==F & Alu==F)]][chr=="chr1"]
fraction_overlapN2(tmp, M4_locs, x1="Unexplained Zcw UT", y1="CNN7_locs", shift=1e8)
```
